"""Main script for airplane detection in satellite imagery."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Detect airplanes in satellite imagery using YOLO models"
    )
    parser.add_argument(
        "input",
        type=str,
    ...
    if input_path.is_file():
        ...
        if result:
            ...
            print(f"{'='*60}")
            print(f"Number of airplanes detected: {summary['num_detections']}")
        ...
        for image_path, result in zip(image_paths, results):
            if result:
                ...
                print(f"{image_path.name}: {num_det} airplane(s) detected")
        
        print(f"\n{'='*60}")
        print(f"Total airplanes detected across all images: {total_detections}")
        print(f"{'='*60}")
